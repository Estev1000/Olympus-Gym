...
function submitLogin() {
    ...
    if (dni.length === 0) {
        showMessage('Por favor ingrese un DNI para Olympus Gym', 'error');
        return;
    }

    if (dni.length < 5) {
        showMessage('DNI inválido para Olympus Gym (muy corto)', 'error');
        return;
    }
    ...
    setTimeout(() => {
        ...
        if (user) {
            ...
            if (!accesoPermitido) {
                ...
                showMessage(`Hola ${user.nombre}, acceso denegado a Olympus Gym: ${partes.join(' y ')}.`, 'error');
                playSound('error');
                ...
                if (ultimoEsPendiente) {
                    showMessage(`¡Bienvenido/a ${user.nombre} a Olympus Gym! Acceso Permitido (PAGO PENDIENTE).`, 'warning');
                    playSound('warning');
                    startBtn.innerHTML = '<i class="fa-solid fa-circle-exclamation"></i> Pendiente';
                } else {
                    showMessage(`¡Bienvenido/a ${user.nombre} a Olympus Gym! Acceso Permitido.`, 'success');
                    playSound('success');
            ...
            // NOT FOUND
            showMessage('DNI no encontrado en el sistema de Olympus Gym.', 'error');
            playSound('error');
